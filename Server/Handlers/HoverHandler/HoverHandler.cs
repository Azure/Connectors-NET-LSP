...
public class HoverHandler(SdkIndex? sdkIndex, BufferManager bufferManager, ConnectionsService connectionsService, ApiService apiService, LSPStore lspStore, ITelemetryService telemetryService) : HoverHandlerBase
{
    ...
    private void EnsureRegistryInitialized()
    {
        ...
        DynamicOperationsRegistry.Initialize(sdkIndex);
    }
    ...
    public override async Task<Hover?> Handle(HoverParams request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            string? symbolInfo = await AnalyzeSymbolAtPositionAsync(documentText, request.Position, documentFilePath, cancellationToken);
    ...
    /// <remarks>
    /// This method includes fundamental .NET types (string, object, Console), LINQ support, async/await support,
    /// and Microsoft Extensions libraries commonly used in modern .NET applications (DI, Configuration, Logging, Hosting).
    /// References are added using both direct assembly locations and trusted platform assemblies.
    /// </remarks>
    ...
    private async Task<string?> AnalyzeSymbolAtPositionAsync(string documentText, Position position, string? documentFilePath, CancellationToken cancellationToken)
    {
        try
        {
            // Parse the document
            ...
            // Add project NuGet references for proper method symbol resolution
            if (!string.IsNullOrEmpty(documentFilePath))
            {
                int nugetCount = AddProjectNuGetReferences(references, referencePaths, documentFilePath);
                if (nugetCount > 0)
                {
                    await Console.Error.WriteLineAsync($"[HoverHandler] Added {nugetCount} NuGet reference(s) from project");
                }
            }
    ...
    /// </summary>
    private static int AddProjectNuGetReferences(List<MetadataReference> references, HashSet<string> seenPaths, string documentFilePath)
    {
            ...
            if (root.TryGetProperty("targets", out JsonElement targetsElement))
            {
                foreach (JsonProperty target in targetsElement.EnumerateObject())
                {
                    foreach (JsonProperty package in target.Value.EnumerateObject())
                    {
                        ...
                        foreach (JsonProperty asset in compileElement.EnumerateObject())
                        {
                            string assetRelPath = asset.Name;
                            if (!assetRelPath.EndsWith(".dll", StringComparison.OrdinalIgnoreCase))
                            {
                                continue;
                            }

                            foreach (string pkgFolder in packageFolders)
                            {
                                string fullPath = Path.Combine(pkgFolder, libraryPath, assetRelPath);
                    ...
                    // TODO(#11): This only processes the first target framework in project.assets.json.
                    // In multi-targeted projects, the first TF may not match the document's TF,
                    // which can result in missing or wrong metadata references.
